...
__global__ void ising(int *G, int *H, double *w, int n){
    ...
    int i_start = (threadIdx.x + blockIdx.x * blockDim.x)*SPINS_PER_THREAD_DIM;
    int j_start = (threadIdx.y + blockIdx.y * blockDim.y)*SPINS_PER_THREAD_DIM;
